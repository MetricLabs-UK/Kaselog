...
return [
    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                /*
                 * Deliberately storage_path('app') only, not base_path() (the
                 * whole codebase) — the application code is already in git;
                 * what genuinely can't be recreated from source control is
                 * client-uploaded and generated files (matter documents,
                 * generated documents, precedent templates — all under the
                 * 'documents' disk, config/filesystems.php) plus anything
                 * else under storage/app. .env is deliberately NOT included
                 * here — see this file's own 'password'/'encryption' keys
                 * below for why bundling secrets into the same archive
                 * needs encryption turned on first, and the manual
                 * checklist for the decision on whether to do that at all.
                 */
                'include' => [
                    storage_path('app'),
                ],
             ...
             */
            /*
             * Explicit connection names, not env('DB_CONNECTION') — the
             * audit trail (item 14's 'audit' connection, a genuinely
             * separate physical database, kase_audit) must never be the one
             * thing that isn't backed up alongside the main app database.
             */
            'databases' => [
                'mysql',
                'audit',
    ...
    'notifications' => [
        ...
        'mail' => [
            // env()'s own default param only applies when the key is wholly
            // absent — this app's .env convention leaves unset secrets as
            // blank `KEY=` lines (present, empty string), which env() would
            // otherwise pass straight through and fail spatie/backup's own
            // "is this a valid email" config validation on every boot.
    ...
    'monitor_backups' => [
        [
            ...
            'health_checks' => [
                ...
                // every backup as unhealthy the moment it's a day old.
    ...
    'cleanup' => [
        ...
        /*
         * Tuned for a weekly cadence (routes/console.php runs the backup
         * once a week, not daily) — the package's day-granularity tiers
         * (keep_all_backups_for_days / keep_daily_backups_for_days) don't
         * apply to anything here since there's never more than one backup
         * per week to thin out; set to 0 rather than left at package
         * defaults that assume daily backups. Recommendation to confirm:
         * ~12 weekly backups (3 months) kept in full, then one per month
         * for a year, then yearly for 2 years. Adjust
         * delete_oldest_backups_when_using_more_megabytes_than once real
         * backup sizes are known — 5000 (5GB) is a placeholder ceiling, not
         * a measured one.
         */
        'default_strategy' => [
